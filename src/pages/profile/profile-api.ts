...
/**
 * One page of the account's posts, oldest-bound by `maxId`.
 *
 * `exclude_replies` is what makes this "posts" rather than "posts and
 * replies". Akkoma applies it to self-replies too (measured 2026-08-30: a
 * self-reply present without the flag is gone with it), so a thread its own
 * author continued shows only its opening post here. Taken as the endpoint's
 * meaning — filtering client-side would drop items out of a page whose length
 * is what decides whether more exist.
 */
